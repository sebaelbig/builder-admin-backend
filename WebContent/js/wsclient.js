var wsclient = (function() {
	...
	var ws = null;
	var wsURI = 'ws://' + location.host + '/horus_restfull/chat';
	...
	function connect(userName) {
		...
		function processMessage(message) {
			if (message.messageInfo) {
				...
				addMessage(message.messageInfo.from,
						message.messageInfo.message,
						cleanWhitespaces(message.messageInfo.from)
								+ 'conversation');
	...
	function createConversationPanel(name) {
		...
		$(
				'<p class="messages"></p><textarea id="' + conversationId
						+ 'message"></textarea>').appendTo(conversationPanel);
	...
	function createSendButton(name) {
		...
		button
				.click(function() {
					...
					var message = document.getElementById(conversationId
							+ 'message').value;
	...
	function addMessage(from, message, conversationPanelId) {
		...
		$(
				'<div class="message"><span><b>' + from
						+ '</b> dice:</span><p>'
						+ $('<p/>').text(message).html() + '</p></div>')
				.appendTo(messages);
