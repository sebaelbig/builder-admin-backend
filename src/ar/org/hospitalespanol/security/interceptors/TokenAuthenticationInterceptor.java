package ar.org.hospitalespanol.security.interceptors;
...
public class TokenAuthenticationInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if (requiereAutorizacion && !tieneAutorizacion) {
			...
			this.logger.error("No tiene autorización para realizar esta petición"
					+ "ip {}, to {}", request.getRemoteAddr(), request.getRequestURI());

			//NO lo trae, acceso prohibido (CODE 43)
			response.sendError(403);
			
			this.webContextHolder.setMensajes("No tiene autorización para realizar esta petición.");
			...
			if (userVo != null) {
				...
				DAO_Utils.error(this.logger, "TokenAuthenticationInterceptor", "preHandle", "-usuario no encontrado-", "ERROR!! Token invÃ¡lido - Accion no permitida, no se encontró el usuario con el token suministrado.");

				//NO lo trae, acceso prohibido (CODE 43)
				response.sendError(403);

				this.webContextHolder.setMensajes("No tiene autorización para realizar esta petición.(Token invalido)");
